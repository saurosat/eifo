...
local function tobuf(s, len, ct, sizeof_ct)
	if type(s) == 'string' then
        local len = math.min(len or (1/0), #s / (sizeof_ct or 1))
        ngx.log(ngx.DEBUG, "s = "..s..", len = "..len)

		return s, ffi.cast(ct or uint8_ptr, s), len
...
function utf8.encode(buf, len, out, outlen, repl)
	...
	if not out then --allocate output buffer
		outlen = outlen or byte_count(buf, len, repl)
